#' CircE-modified, determinant (robust)

#' @description This is a help function that performs Browne's procedure CIRCUM
#' on the items to obtain item angles and communalities to be inserted into
#' ClusterCirc. The function is a modified version of circe.bfgs from the CircE
#' package (Grassi et al., 2010). This version is robust in cases of matrix
#' indeterminacy.

circe_det001 <- function(R,
                         v.names,
                         m,
                         N,r,
                         equal.com,
                         ...
                         start.values,
                         ci.level=0.95,
                         factr=1e10,pgtol=0,lmm = 3,
                         iterlim=100, upper=NULL,lower=NULL,print.level=2,file=NULL,title="Circumplex Estimation",try.refit.BFGS=FALSE, verbose = FALSE){
  ...
  objective1max<-
    function(par){
      ...
      Dz=diag(z);
      buggi=Dz%*%(Pc+Dv)%*%Dz
      while (det(buggi) <= 0.001) {diag(buggi) <- diag(buggi) + 0.002}
      f=-1*(sum(diag(R%*%solve(buggi)))+log(det(buggi))-log(det(R))-p);
      S1=buggi;S2=diag(1/sqrt(diag(S1)))%*%Dz;S=S2%*%(Pc+Dv)%*%S2
      attributes(f)=list(f=f,S=S,Cs=buggi,Pc=Pc)
      f}
  ...
  objective1hess<-
    function(par){
      ...
      S=buggi;S2=diag(1/sqrt(diag(S)))%*%Dz;S1=S2%*%(Pc+Dv)%*%S2
      hessian=matrix(0,length(par),length(par))
